...
// Change MatC to "X - λ*vecx*(t^vecx)"(≡B)
...
int main(int argc, char const* argv[]) // ./program "filename" "numfindvectors"
{
   ...
   // make convertedtMatX ,if principal components are displayed.
   if(argc == 4) {
   ...
   for(nfvector = 1; nfvector <= numfindvectors; nfvector++) {
      make_MatB(MatC, vecx, eigen, dim); // Change MatC to "C - λ*vecx*t^vecx"(≡B)
      for(row = 0; row < dim; row++) {
         vecx[row] = 1.0;
      }
